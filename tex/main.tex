...
\begin{equation}
    A_{nk} =
    \begin{cases}
        (2n + 1)^{1/2} (2k + 1)^{1/2} & \text{wartości poniżej przekątnej} \\
        n + 1                         & \text{wartości na przekątnej}      \\
        0                             & \text{wartości powyżej przekątnej}
    \end{cases}
    \label{equ:hippo_matrix}
\end{equation}

Zbudowanie macierzy w taki sposób prowadzi do zdecydowanie lepszych wyników niż zainicjalizowanie jej losowo. wielomian Lagrange'a
...
%     \begin{tabular}{|l|c|c|}
%         \hline
%         \textbf{Architektura}       & \textbf{Transformer} & \textbf{Mamba} \\ \hline
%         \textbf{Szybkość predykcji} & $O(n)$               & $O(1)$         \\ \hline
%         \textbf{Szybkość treningu}  & $O(n^2)$             & $O(n)$         \\ \hline
%     \end{tabular}
%     \caption{Porównanie transformera i mamby.}
