...
Zależności pomiędzy słowami w mechanizmie uwagi można przedstawić jako graf pełny skierowany, gdzie wierzchołkami sa poszczególne słowa lub inne tokeny z okna czasowego. Krawędziami są wartości, które mówią o tym, jak istotne dla danego słowa są wszystkie inne. Jest to o zdecydowanie lepsze podejście niż w rekurencyjnych sieciach neuronowych lub LSTM-ach, gdzie poszczególne zależności dla danego słowa są przekazywane jedynie w wektorze, który kompresuje kontekst ze wszystkich innych poprzednich słów. Z tego powodu mechanizm uwagi pozwala na bardziej dynamiczną kontrolę i monitoring tego w jaki sposób zależności pomiędzy tokenami formują się podczas treningu.
