...
Jako reprezentanta architektury transformerowej została wybrana ta, której używa model GPT-2 \cite{gpt2}. Został stworzony przez OpenAI i~jest to obecnie ich najnowszy model, który jest udostępniony jako \textit{open source}. Jest oparty o~architekturę \textit{decoder-only}.
...
Duże modele lepiej poradziły sobie z~odtworzeniem odpowiedniej tonacji, jednak różnice są~niewystarczające, aby ocenić, czy używanie danej architektury ma \textcolor{blue}{istotny wpływ} na~jakość predykcji.
