...
\begin{equation}
    ...
    \label{equ:ar_model}
\end{equation}
Przykładowe podejście do auto-regresyjnej generacji sekwencji pokazano w równaniu \ref*{equ:ar_model}. Algorytm jest agnostyczny w stosunku do typu danych, więc w $X$ mogą być zarówno wszystkie słowa, litery lub elementy notacji muzycznej.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\linewidth]{./img/traning_modeli_midi.pdf}
    \caption{Wykresy treningu modeli.}\label{fig:midi_train}
\end{figure}

Na grafice \ref*{fig:midi_train} przedstawiono \textit{loss} trenigu modeli. Ponownie, w celu lepszej wizualizacji ograniczono
\begin{comment}
Tutaj trzeba ten obrazek \ref*{fig:midi_train} troche naprawić bo nie zgadzają się training stepy więc w sumie nic nie widać
\end{comment}
...
\begin{minted}[breaklines,fontsize=\tiny,breaksymbolleft=]{text}
