...
\begin{itemize}
    \item \texttt{n\_embed} - Dimensionality of the embeddings and hidden states.
    \item \texttt{n\_layer} - Number of hidden layers in the Transformer encoder.
    \item \texttt{n\_head} - Number of attention heads for each attention layer in the Transformer encoder.
    \item \texttt{n\_inner} - Dimensionality of the inner feed-forward layers.
\end{itemize}

Parametry modeli Mamba:
\begin{itemize}
    \item \texttt{hidden\_size} - Dimensionality of the embeddings and hidden states.
    \item \texttt{state\_size} -  Shape of the state space latents.
    \item \texttt{num\_hidden\_layers} - Number of hidden layers in the model.
\end{itemize}

\begin{comment}
W tym miejscu będą wykresiki treningu modeli, i tego rodzaju rzeczy, czas treningu, ilość epok i tak dalej + opis czym są te parametry w tabelkach
\end{comment}

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\linewidth]{./img/traning_modeli_midi.pdf}
    \caption{Wykresy treningu modeli MIDI.}\label{fig:midi_train}
\end{figure}
