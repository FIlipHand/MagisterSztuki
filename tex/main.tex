...
Aby upewnić się nie jest to jest domyślne zachowanie należy poprosić model o ocenę fragmentu, który został specjalnie przygotowany, aby nie brzmiał dobrze dla ludzkiego ucha. Zapis tego fragmentu został zapisany na grafice \ref*{fig:gen_bad}. Dodatkowo, pomimo tego że zapis jest poprawny w notacji ABC, niektóre takty zawierają więcej dźwięków niż jest to przewidziane w metrum. Odpowiedź modelu przedstawiono we fragmencie \ref*{code:chat_prompt2}.
...
\begin{figure}[ht!]
    ...
    \end{center}
    \caption{Specjalnie stworzony ``brzydki'' fragment muzyki.}\label{fig:gen_bad}
\end{figure}
